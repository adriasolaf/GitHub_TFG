...
if (r1n == 0.0 || r2n == 0.0), flag=201; dbg.flag = flag; return; end % Status code: bad input position vectors
if (tof <= 0.0), flag=202; dbg.flag = flag; return; end % Status code: negative tof
if (mu <= 0.0), flag=203; dbg.flag = flag; return; end % Status code: negative mu
if (tol <= 0.0), flag=206; dbg.flag = flag; return; end % Status code: negative tolerance
...
if mr > 0
    ...
    [ ~, Tmin, ~, iter1 ] = Lambert_Izzo_2015_X_Tmin( r1, r2, mu, lw, mr );
    Tmin = Tmin * sqrt(2 * mu / (s*s*s)); %  Normalize magnitude
    dbg.iter1 = iter1;

    % Check if tof is too low for the required Tmin
    if Tmin > t, flag = 700; dbg.flag = flag; return; end
...
for i=1:ni
    ...
    if i >= ni, flag = 301; dbg.flag = flag; return; end
    
end

% Save initial guess error
dbg.initerr = x - dbg.x0;
